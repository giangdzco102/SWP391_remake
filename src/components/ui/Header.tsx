...
export function Header({ pending, darkMode, setDarkMode }: any) {
  ...
  const navLinks = useMemo(() => [
    ...
    ...(user?.roles.includes("REVIEWER") ? [{ label: "Kiểm duyệt", page: "reviewer-dash", badge: pending?.length }] : []),
    ...(user?.roles.includes("EDITOR") ? [{ label: "Nhiệm vụ", page: "editor-dash" }] : []),
    ...(user?.roles.includes("AUTHOR") ? [{ label: "Tác phẩm của tôi", page: "my-stories" }] : []),
    ...(user?.roles.includes("ADMIN") ? [{ label: "⚙ Admin", page: "/adminDashboard" }] : []),
  ], [user?.roles, pending?.length]);

  // ── Helper: close mobile then run fn ─────────────────────────────────────
