...
function ReportResolveModal({ report, onSubmit, onClose }: { report: any; onSubmit: (p: any) => void; onClose: () => void }) {
  ...
  const ACTIONS = [
    { value: "WARN_ONLY",      label: "⚠️ Chỉ ghi nhận",        desc: "Đánh dấu đã xử lý, không thực hiện thêm hành động" },
    { value: "HIDE_CONTENT",   label: "🙈 Ẩn nội dung",         desc: "Ẩn story/chapter/comment bị báo cáo" },
    { value: "DELETE_CONTENT", label: "🗑 Xóa nội dung",         desc: "Xóa hoàn toàn nội dung bị báo cáo" },
    { value: "BAN_USER",       label: "🚫 Khóa tài khoản",      desc: "Chỉ khóa tài khoản tác giả, không xử lý nội dung" },
    { value: "HIDE_AND_BAN",   label: "🙈🚫 Ẩn + Khóa",         desc: "Ẩn nội dung và khóa tài khoản tác giả" },
    { value: "DELETE_AND_BAN", label: "🗑🚫 Xóa + Khóa",        desc: "Xóa nội dung và khóa tài khoản tác giả" },
  ];

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "#fff", borderRadius: 14, padding: 28, maxWidth: 520, width: "90%", boxShadow: "0 20px 60px rgba(0,0,0,.3)", maxHeight: "90vh", overflowY: "auto" }}>
        <h3 style={{ margin: "0 0 4px", fontSize: 16, fontWeight: 700 }}>Xử lý báo cáo #{report.id}</h3>
        <p style={{ margin: "0 0 14px", fontSize: 13, color: "#6b7280" }}>
          Từ <strong>{report.reporterName}</strong> · {report.targetType} #{report.targetId}
        </p>
        {report.reason && (
          <div style={{ background: "#fef3c7", borderRadius: 8, padding: "8px 12px", marginBottom: 14, fontSize: 13, color: "#92400e" }}>
            💬 {report.reason}
          </div>
        )}
...
function ReportDetailModal({ report, onResolve, onClose }: { report: any; onResolve: (r: any) => void; onClose: () => void }) {
  ...
  const renderTarget = () => {
      ...
      return (
        <div>
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
            ...
            <span style={{ fontWeight: 600, fontSize: 13, color: "#1c1512" }}>{name !== "—" ? name : `Người dùng #${target?.userId ?? "?"}`}</span>
            {createdAt && <span style={{ fontSize: 11, color: "#9ca3af" }}>{new Date(createdAt).toLocaleDateString("vi-VN")}</span>}
            <span style={{ fontSize: 11, color: "#9ca3af", marginLeft: "auto" }}>ID bình luận: #{report.targetId}</span>
          </div>
          {content ? (
            <div style={{ fontSize: 14, color: "#374151", lineHeight: 1.65, background: "#f9fafb", borderRadius: 8, padding: "10px 12px", border: "1px solid #e5e7eb" }}>
              {content}
            </div>
          ) : (
            <div style={{ fontSize: 13, color: "#9ca3af", fontStyle: "italic", padding: "8px 0" }}>
              Không tìm thấy nội dung bình luận (ID: {report.targetId}).
            </div>
          )}
        </div>
      );
    ...
    if (report.targetType === "CHAPTER") return (
      <div>
        <div style={{ fontWeight: 700, fontSize: 15, color: "#1c1512", marginBottom: 4 }}>{target.title ?? `Chương #${report.targetId}`}</div>
        {target.storyTitle && <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 4 }}>📖 Thuộc truyện: <strong>{target.storyTitle}</strong></div>}
        {target.chapterOrder != null && <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 4 }}>📌 Chương {target.chapterOrder}</div>}
        {target.coinPrice > 0 && <div style={{ fontSize: 12, color: "#d97706", marginBottom: 4 }}>🔒 Chương VIP · {target.coinPrice} xu</div>}
        {target.content && (
          <p style={{ margin: "8px 0 0", fontSize: 13, color: "#374151", lineHeight: 1.55, display: "-webkit-box", WebkitLineClamp: 4, WebkitBoxOrient: "vertical", overflow: "hidden", background: "#f9fafb", borderRadius: 6, padding: "8px 10px", border: "1px solid #e5e7eb" }}>
            {target.content}
          </p>
        )}
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 16, padding: 0, maxWidth: 560, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,.3)", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ padding: "20px 24px" }}>
          ...
          <div style={{ marginBottom: 18 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#374151", marginBottom: 6 }}>💬 Lý do báo cáo</div>
            <div style={{ background: "#fffbeb", border: "1.5px solid #fde68a", borderRadius: 8, padding: "12px 14px", fontSize: 14, color: "#92400e", lineHeight: 1.65 }}>
